# Rust MVC API - Podman Helper Script (PowerShell)
# Usage: .\podman-helper.ps1 [build|run|stop|logs|test|clean]
...
function Show-Help {
    Write-Host "Rust MVC API - Podman Helper" -ForegroundColor Cyan
    Write-Host ""
    Write-Host "Usage: .\podman-helper.ps1 [command]" -ForegroundColor Yellow
    Write-Host ""
    Write-Host "Commands:" -ForegroundColor Green
    Write-Host "  build   - Build the Podman image"
    ...
    Write-Host "  .\podman-helper.ps1 build"
    Write-Host "  .\podman-helper.ps1 run"
    Write-Host "  .\podman-helper.ps1 test"
    Write-Host "  .\podman-helper.ps1 stop"
}

function Build-Image {
    Write-Host "🔨 Building Podman image..." -ForegroundColor Yellow
    podman build -t "${IMAGE_NAME}:latest" -f Podmanfile .
...
function Test-API {
    ...
    try {
        ...
        $projects = Invoke-WebRequest -Uri "http://localhost:$PORT/api/projects" -UseBasicParsing
...
function Clean-Up {
    ...
    try {
        podman stop $CONTAINER_NAME 2>$null
        podman rm $CONTAINER_NAME 2>$null
        podman rmi "${IMAGE_NAME}:latest" 2>$null
