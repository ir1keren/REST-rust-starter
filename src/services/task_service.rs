...
use crate::repositories::{TaskRepository, ProjectRepository};
...
impl TaskService {
    pub fn new(task_repository: Arc<TaskRepository>, project_repository: Arc<ProjectRepository>) -> Self {
        Self {
            task_repository,
            project_repository,
        }
    ...
    pub async fn create_task(&self, create_data: TaskCreate) -> Result<Task, ApiError> {
        ...
        if create_data.title.len() > 200 {
            return Err(ApiError::validation_error("Task title cannot exceed 200 characters"));
        }
        
        if let Some(ref description) = create_data.description {
            if description.len() > 1000 {
                return Err(ApiError::validation_error("Task description cannot exceed 1000 characters"));
            }
        }
    ...
    pub async fn delete_task(&self, id: Uuid) -> Result<(), ApiError> {
        let deleted = self.task_repository
            .delete(&id)
            .map_err(|e| ApiError::repository_error(&e))?;
